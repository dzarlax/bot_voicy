...
bot.start((ctx) =>
  ctx.reply("Привет! Отправьте мне видео, аудио файл или ссылку на видео.")
);
bot.help((ctx) =>
  ctx.reply("Отправьте видео или аудио файл для транскрибации.")
);
  ...
  try {
    ...
    if (content.length > 4096) {
      await ctx.replyWithDocument({
        ...
        filename: `${transcriptionDirName}-transcription.txt`,
      });
    } else {
      await ctx.reply(content);
    }
    // Удаление файла и папки после отправки
    await deleteTranscriptionResults(transcriptionDir, filePath); 
  } catch (error) {
...
async function deleteTranscriptionResults(transcriptionDir, originalFilePath) {
  try {
    ...
    console.log(
      `Папка с результатами ${transcriptionDir} и содержимое успешно удалены.`
    );
  } catch (error) {
    console.error(
      "Ошибка при удалении исходного файла или папки с результатами транскрибации:",
      error
    );
  }
}
